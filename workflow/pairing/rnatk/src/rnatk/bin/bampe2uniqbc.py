#!/usr/bin/env python
"""
Remove PE reads having same location (chr, start, end, R1strand) by barcode sequences.
...
import argparse
import pysam
from rnatk.ios.bam import ibampe_chromStartEndR1Strand, igroup_barcode
import logging
_logger = logging.getLogger(__name__)
from rnatk import __version__


def bampe2uniqbc(inbam, outbam, maxtop=1, bclen=4):
    assert os.path.exists(inbam)
    bamin = pysam.Samfile(inbam, "rb")
    oh = pysam.Samfile(outbam, "wb", template=bamin)
    for sig, rli in ibampe_chromStartEndR1Strand(bamin):
        rns = set()
        reads = [pe['R1'] for pe in rli]
        for bc, grpreads in igroup_barcode(reads, bclen):
            for s in grpreads[:maxtop]:
                rns.add(s.qname)
        for pe in rli:
            if pe['R1'].qname in rns:
                oh.write(pe["R1"])
                oh.write(pe["R2"])
    oh.close()
...
def parse_args(args):
    ...
    parser.add_argument(
        "inbam",
        type=str,        
        help="Input PE Bam file, position sorted with samtools",
        metavar="input.bam")
    parser.add_argument(
        "outbam",
        type=str,        
    ...
    parser.add_argument(
        ...
        type=int,        
        dest="bclen",
        help="Length of barcode 'read|barcode' [4]",
        default=4)
    parser.add_argument(
        '--version',
        action='version',
...
def main(args):
    ...
    bampe2uniqbc(args.inbam, args.outbam, maxtop=1, bclen=4)
