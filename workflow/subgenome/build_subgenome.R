...
subgenome_num <- read_delim("analysis/subgenome/subgenome/subgenome.group.tsv", "\t", escape_double = FALSE, trim_ws = TRUE)
...
ggsave(file.path(oSG, "subgenome.multi_single.cnt.pdf"), p, width = 7, height = 4, units = "cm")


################################################
## Figure 2F
target_sn_js <- multi_js_subgenome[grep("28525-28576", multi_js_subgenome$LeaderJuncList),]
write_tsv(target_sn_js, file.path(out_dir, "subgenome", "SN.top.sourceData.tsv"))
target_sn_js$Structure <- paste(target_sn_js$LeaderSequence, target_sn_js$ORF1ab_S2N, target_sn_js$S2N_S2N)
target_sn_js$Structure <- factor(
  target_sn_js$Structure, 
  levels = rev(c(
    "1 0 0", 
    "1 0 1", 
    "0 1 0", 
...
plot_str_li <- build_str_df(target_sn_js)
gene_ref <- read_delim(file.path(data_dir, "genome/WIV04.bed"), "\t", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)
gene_ref_df <- data.frame(Name=gene_ref$X4, Start=gene_ref$X2, End=gene_ref$X3)
gene_ref_df$Mid <- (gene_ref_df$Start + gene_ref_df$End) / 2
gene_ref_df <- gene_ref_df[order(gene_ref_df$Start), ]
...
ggsave(file.path(out_dir, "subgenome", "subgenome.SN.str.pdf"), p, width = 12, height = 5, units = "cm")
