...
################################################
## Figure 1G (Vero E6, 48h)

# Show all NGS junctions and highight enriched ones
genome_size <- 29891
p <- ggplot() +
  geom_point(data=NGS_JS_merged, mapping=aes(x=Start, y=End), color="#92C5DE", alpha=0.3, size=0.1) +
...
jun_all_df$SubgenomeType <- sapply(jun_all_df$End, function (end_site) {
...
jun_all_df$SubgenomeType[jun_all_df$Start>100] <- NA
jun_all_df$SubgenomeType[jun_all_df$End<2000] <- NA
jun_all_df$Total <- jun_all_df$Rep1 + jun_all_df$Rep2
top_js_df <- jun_all_df %>% group_by(SubgenomeType) %>% filter(Total==max(Total))
top_js_df <- na.omit(top_js_df)
top_js_df <- top_js_df[top_js_df$SubgenomeType!="Other",]
top_js_df$IsTop <- TRUE
jun_all_df <- left_join(jun_all_df, top_js_df[,c("Start", "End", "IsTop")])
jun_all_df$IsTop[is.na(jun_all_df$IsTop)] <- FALSE

p <- ggplot() +
  geom_point(data=jun_all_df, mapping=aes(x=log10(Rep1+1), y=log10(Rep2+1)), color="grey70", alpha=0.3, size=0.4) +
  geom_point(data=jun_all_df[jun_all_df$Enriched, ], mapping=aes(x=log10(Rep1+1), y=log10(Rep2+1)), color="red", size=0.5) +
  geom_text(data=jun_all_df[jun_all_df$IsTop,], mapping=aes(x=log10(Rep1+1), y=log10(Rep2+1), label=SubgenomeType), size=1.8, color="black") +
  labs(x="#Reads in Rep1", y="#Reads in Rep2") +
  scale_x_continuous(breaks = c( log10(c(1e1, 1e3, 1e5)+1)), labels = c("1E1", "1E3", "1E5")) +
  scale_y_continuous(breaks = c( log10(c(1e1, 1e3, 1e5)+1)), labels = c("1E1", "1E3", "1E5")) +
  theme_bw() +
  theme(text = element_text(family="ArialMT", color = "black", size = 6),
        axis.line = element_line(color = "black"),
        axis.text = element_text(color = "black"),
...
################################################
## Figure S3B (Vero E6, 48h)

# Show reproduicity between replicates
p <- ggplot() +
  geom_point(data=nanopore_JS_merged, mapping=aes(x=Start, y=End), color="#92C5DE", size=0.1, alpha=0.3) +
...
all_consistent_js$Nanopore_cnt <- all_consistent_js$Nanopore_rep1_Cnt + all_consistent_js$Nanopore_rep2_Cnt
donor_cnt <- all_consistent_js %>% group_by(Start) %>% summarise(ReadNum=sum(Nanopore_cnt))
acceptor_cnt <- all_consistent_js %>% group_by(End) %>% summarise(ReadNum=sum(Nanopore_cnt))
p <- ggplot() +
  geom_linerange(data=donor_cnt, mapping=aes(x=Start, ymin=0, ymax=ReadNum), size=0.3, color="black") +
  scale_y_log10() +
  theme_bw() +
  lims(x=c(0, genome_size)) +
  theme(text = element_text(family="ArialMT", color = "black", size = 6),
        axis.line = element_line(color = "black"),
        axis.text = element_text(color = "black"),
        axis.ticks = element_line(color = "black"),
        panel.grid = element_blank())
ggsave(file.path(oNGSION, "Nanopore.Donor.JS.cnt.pdf"), p, width = 7, height = 3.5, units = "cm")

p <- ggplot() +
  geom_linerange(data=acceptor_cnt, mapping=aes(x=End, ymin=0, ymax=ReadNum), size=0.3, color="black") +
  scale_y_log10() +
  theme_bw() +
  lims(x=c(0, genome_size)) +
  theme(text = element_text(family="ArialMT", color = "black", size = 6),
        ...
        axis.ticks = element_line(color = "black"),
        panel.grid = element_blank())
ggsave(file.path(oNGSION, "Nanopore.Acceptor.JS.cnt.pdf"), p, width = 7, height = 3.5, units = "cm")
